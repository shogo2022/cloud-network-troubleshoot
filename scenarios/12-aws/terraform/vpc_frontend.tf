#Create vpc
resource "aws_vpc" "handsonlab_frontend_vpc" {
  cidr_block = "10.0.0.0/16"
  tags = {
    Name       = "handsonlab_frontend_vpc"
    Handsonlab = true
  }
}

#Create internet gateway
resource "aws_internet_gateway" "handsonlab_frontend_igw" {
  vpc_id = aws_vpc.handsonlab_frontend_vpc.id
  tags = {
    Name       = "handson_frontend_igw"
    Handsonlab = true
  }
}
...
resource "aws_route_table" "handsonlab_public_rt" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.handsonlab_frontend_igw.id
  }
  tags = {
