...
# security group rule for db backend access from web frontend
resource "aws_security_group_rule" "sql-access" {
  type            = "ingress"
  from_port       = 3306
  to_port         = 3306
  protocol        = "tcp"
  source_security_group_id = aws_security_group.handsonlab_db_sg.id

  security_group_id = aws_security_group.handsonlab_db_sg.id
}
